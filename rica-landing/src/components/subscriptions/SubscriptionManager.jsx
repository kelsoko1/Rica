...
const SubscriptionManager = () => {
  ...
  const calculatePrice = (planId, cycle) => {
    const plan = subscriptionService.subscriptionPlans[planId];
    if (!plan) return 0;
    
    // Handle Pay As You Go plan separately
    if (planId === 'payAsYouGo') {
      return {
        baseFee: plan.price.monthly,
        includedTokens: plan.price.tokens,
        additionalTokenPrice: plan.price.additionalTokenPrice
      };
    }
    
    return plan.price[cycle] || plan.price.monthly;
  };
  ...
  // Render plan price
  const renderPlanPrice = (plan, cycle) => {
    const price = calculatePrice(plan.id, cycle);
    const isAnnual = cycle === 'annual';
    const isPayAsYouGo = plan.id === 'payAsYouGo';
    
    if (isPayAsYouGo) {
      return (
        <Box>
          <PlanPrice>
            {formatCurrency(price.baseFee, 'USD')}
            <Typography variant="body2" color="text.secondary" component="span">
              /month
            </Typography>
          </PlanPrice>
          <Typography variant="caption" color="text.secondary">
            + {price.includedTokens} tokens included
          </Typography>
        </Box>
      );
    }
  ...
  const handleCreateSubscription = () => {
    ...
    const isPayAsYouGo = selectedPlan === 'payAsYouGo';
    ...
    try {
      ...
      const subscription = subscriptionService.createSubscription({
        ...
        planId: selectedPlan,
        billingCycle: isPayAsYouGo ? 'monthly' : billingCycle, // Force monthly for Pay As You Go
        currency: 'USD',
        paymentMethod: 'card',
            ...
            <Grid container spacing={3}>
              <Grid item xs={12} md={6}>
                ...
                <Typography variant="subtitle2" color="text.secondary" gutterBottom sx={{ mt: 2 }}>
                  {activeSubscription.planId === 'payAsYouGo' ? 'Base Fee' : 'Price'}
                </Typography>
                <Typography variant="h6" gutterBottom>
                  {formatCurrency(activeSubscription.price, activeSubscription.currency)}
                  {activeSubscription.planId === 'payAsYouGo' ? (
                    <>
                      <Typography component="span" color="text.secondary">
                        {' '}/ month
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                        Includes: {activeSubscription.metadata?.tokens || 100} tokens
                      </Typography>
                    </>
                  ) : (
                    <Typography component="span" color="text.secondary">
              ...
              <Grid item xs={12} md={6}>
                {activeSubscription.planId === 'payAsYouGo' ? (
                  <>
                    ...
                    <Typography variant="h6" gutterBottom>
                      {activeSubscription.metadata?.tokens || 100}
                      <Typography variant="caption" color="text.secondary" display="block">
                        Last topped up: {formatDate(activeSubscription.metadata?.lastTopUp || activeSubscription.createdAt)}
                      </Typography>
                    </Typography>
                  </>
                ) : (
